...
const CategoryCardsGrid: FC = () => {
    ...
    const renderColumn = (column: (typeof CATEGORY_GRID_COLUMNS)[number]) => (
        <div key={column.id} className="flex min-h-0 min-w-0 flex-1 flex-col gap-3 sm:gap-4 h-auto">
            {column.cards.map((card) => (
                <Card key={card.id} variant="dark" className="flex min-h-0 min-w-0 flex-1 flex-col gap-3 rounded-lg p-2 shadow-sm sm:gap-4 sm:p-4">
                    <h3
